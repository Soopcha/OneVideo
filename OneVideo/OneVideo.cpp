...
*/
	...
	//кста целочисленная переменная не может дробь ходить
	...
	cout << "Введите число  ";
	cin >> Var;
	cout << "Вы ввели   " << Var << endl;
	cin >> a >> b;
	cout << "А ещё вы ввели  " << a << b << endl;



	//Операторы (операции)  (в конспекте си есть)
	/* унарные     - нужен 1 операнд    ++ -- и тд
	   бинарные    - нужны два операнда  + - * / %, побитовые,  и тд.
	   тернарные   - нужны три операнда  
	*/

	cin >> a >> b >> c;
	double average;

	average = (a + b + c) / 3;   //получим целую часть только почему-то
	cout << "Среднее арифм 3-ч чисел =   " << average << endl;		

	average =(double) (a + b + c) / 3;   //(double) - операция преведения типов
	cout << "Среднее арифм 3-ч чисел =   " << average << endl;

	cout << "Среднее арифм 3-ч чисел =   " << (double)(a + b + c) / 3 << endl;
